...
:::note
Cloudflare recommends [deploying Astro apps to Workers](/workers/framework-guides/web-apps/astro/) instead of Pages, particularly if you rely on server-side rendering. Workers supports the same `@astrojs/cloudflare` adapter and provides access to additional platform features such as [Durable Objects](/durable-objects/) and [built-in observability](/workers/observability/).
:::
